...
school.get('/:id', async (req, res) => {
    try {
        // Import neccesary repos
        const schoolRepo = getRepository(School)
        const schoolEntity = await schoolRepo.findOne({ where: { id: req.params.id }, relations: ['location', 'principal'] })
...
school.post('/', async (req, res) => {
    try {
        ...
        // Create Principal
        const createdUser = createUser(principal.email, principal.password, 'principal');
        const createdPrincipal = createPrincipal(createdUser, principal.name);

        // Create School
        const schoolEntity = schoolRepo.create({ name, location: locationEntity, principal: createdPrincipal })
        const createdSchool = await schoolRepo.save(schoolEntity)

        res.status(201).send({ message: "Created School and Principal successfully" })
    }
    catch (err) {
